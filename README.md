...
- **大当たり確率はセッション使用率に連動**——Claude をいっぱい使う人ほど当たりやすくなる仕様です。
  ...
  - 使用率は姉妹プラグイン [claude-usage](https://github.com/yuuta1219/claude-usage) のキャッシュ（5分毎更新）を優先して参照します
  - claude-usage を入れていなくても**単体で動きます**（Claude Code の認証情報で usage API から直接取得。数値は `/status` と同じ、トークン消費ゼロ）
  - 使用率が取得できない場合は基本確率 1/99 で動作します
- 演出:
